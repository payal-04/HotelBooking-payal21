...
const RadioButton = ({label,selected = false, onChange = ()=>{ }})=>
{
   return(
    <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
      <input type="radio" name ="sortOpton" checked={selected} onChange={()=>
        onChange(label)} />
    <span className='font-light select-none'>{label}</span>
    </label>
...
const AllRooms = () => {
  ...
  //Hamdle Changes for Filter and Sorting//
  ...
  //Function to Check if a room matches the selected price ranges//
      const matchesPriceRange = (room) =>{
          return selectedFilters.priceRange.length === 0 || selectedFilters.priceRange.some(
     range =>{
    ...
    xl:px-32'>
      <div>
        <div className='flex flex-col items-start text-left'>
            ...
            max-w-174'>Take advantage of our limited-time offers and special packages to 
